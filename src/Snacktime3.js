...
export const Snacktime3 = (props) => {
    ...
    useEffect(() => {
        (async () => {
            const playerListRaw = await fetchAndSplit(`${props.folderPrefix}/players.txt`, p => p.includes('|'));
            ...
            const columnsRaw = await fetchAndSplit(`${props.folderPrefix}/columns.txt`, c => c.length !== 0 && c.length !== 1);
            setColumns(columnsRaw);
            
            const extrasRaw = await fetchAndSplit(`${props.folderPrefix}/extras.txt`, c => c.includes('|'));
    ...
    const renderMethod = () => {    
        ...
        return (<div className="container">
                <h2>{props.title}</h2>
                <div>Tabell for {numberToWeekDay(nextPlayday.getDay())} den {formatDate(nextPlayday)}</div>
                <div>
